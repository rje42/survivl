...
n <- 1e5
set.seed(124)
dat <- cox_samp(n, T = 10, formulas = forms, family = fams, pars = pars, link=link) 
                # method = "rejection")
...
```{r check_XY_mv, eval=TRUE}
ps <- predict(glm(X ~ W + X_l1 + Z1 + Z2, data=dat2l, family=binomial), type="response")
wt <- dat2l$X/ps + (1-dat2l$X)/(1-ps)
svyglm(I(1-Y) ~ W + X, family=binomial(log), design=svydesign(~1, weights=wt, data=dat2l))
```
