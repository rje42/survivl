...
set.seed(123)
n <- 1e4
surv_model <- survivl_model(T=20, formulas=formulas, family=family, pars=pars, link=link)
...
```{r sim_dat2}
set.seed(123)
surv_model2 <- survivl_model(T=20, formulas=formulas, family=family, pars=pars, link=link)
dat <- rmsm(1e4, surv_model2)
datl <- surv_to_long(dat)
```
